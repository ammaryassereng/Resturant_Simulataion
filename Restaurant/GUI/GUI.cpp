...
void GUI::PrintMessage(string TS, int wN, int wG, int wV, int acN, int acG, int acV, int soN, int soG, int soV) const
{
	ClearStatusBar();	//First clear the status bar

	pWind->SetPen(DARKRED);
	pWind->SetFont(18, BOLD, BY_NAME, "Arial");
	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 1.1), TS);

	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 1.2) + 10, "Waiting Orders->");
	pWind->DrawString(140, WindHeight - (int)(StatusBarHeight / 1.2) + 10, "Normal:");
	pWind->DrawString(200, WindHeight - (int)(StatusBarHeight / 1.2) + 10, to_string(wN));
	pWind->DrawString(230, WindHeight - (int)(StatusBarHeight / 1.2) + 10, "Vegan:");
	pWind->DrawString(280, WindHeight - (int)(StatusBarHeight / 1.2) + 10, to_string(wG));
	pWind->DrawString(310, WindHeight - (int)(StatusBarHeight / 1.2) + 10, "VIP:");
	pWind->DrawString(340, WindHeight - (int)(StatusBarHeight / 1.2) + 10, to_string(wV));

	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 1.2) + 30, "Available Cooks->");
	pWind->DrawString(140, WindHeight - (int)(StatusBarHeight / 1.2) + 30, "Normal:");
	pWind->DrawString(200, WindHeight - (int)(StatusBarHeight / 1.2) + 30, to_string(acN));
	pWind->DrawString(230, WindHeight - (int)(StatusBarHeight / 1.2) + 30, "Vegan:");
	pWind->DrawString(280, WindHeight - (int)(StatusBarHeight / 1.2) + 30, to_string(acG));
	pWind->DrawString(310, WindHeight - (int)(StatusBarHeight / 1.2) + 30, "VIP:");
	pWind->DrawString(340, WindHeight - (int)(StatusBarHeight / 1.2) + 30, to_string(acV));


	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 1.2) + 50, "Now serving orders->");
	pWind->DrawString(180, WindHeight - (int)(StatusBarHeight / 1.2) + 50, "Normal:");
	pWind->DrawString(240, WindHeight - (int)(StatusBarHeight / 1.2) + 50, to_string(soN));
	pWind->DrawString(270, WindHeight - (int)(StatusBarHeight / 1.2) + 50, "Vegan:");
	pWind->DrawString(320, WindHeight - (int)(StatusBarHeight / 1.2) + 50, to_string(soG));
	pWind->DrawString(350, WindHeight - (int)(StatusBarHeight / 1.2) + 50, "VIP:");
	pWind->DrawString(380, WindHeight - (int)(StatusBarHeight / 1.2) + 50, to_string(soV));

	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 1.2) + 70, "The Output File is Ready");
}

void GUI::PrintMessage(string TS, int wN, int wG, int wV, int acN, int acG, int acV, int soN, int soG, int soV, Queue<Cook*>& c) const
{
	ClearStatusBar();	//First clear the status bar
	...
	int reference = 200;
	Cook* tmp;
	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 1.2) + 70, "Just Assigned Orders->");
	if (c.isEmpty())
	{
		pWind->DrawString(reference, WindHeight - (int)(StatusBarHeight / 1.2) + 70, "None"); 
		return;
	}
	while (c.dequeue(tmp))
	{
		string sC, sO;
		if (tmp->GetType() == TYPE_VIP)
			sC = "V";
		else if (tmp->GetType() == TYPE_VGAN)
			sC = "G";
		else
			sC = "N";
		if (tmp->GetAssignedOrder()->GetType() == TYPE_VIP)
			sO = "V";
		else if (tmp->GetAssignedOrder()->GetType() == TYPE_VGAN)
			sO = "G";
		else
			sO = "N";
		string s = sC+ to_string(tmp->GetID()) + "(" + sO + to_string(tmp->GetAssignedOrder()->GetID()) + ") ";
		pWind->DrawString(reference, WindHeight - (int)(StatusBarHeight / 1.2) + 70, s);
		reference += 80;
	}
